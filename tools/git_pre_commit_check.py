...
class GitPreCommitChecker:
        ...
        self.expected_structure = {
            'project/src': {
                ...
                'forbidden_files': ['.eslintrc.js', '.prettierrc', 'webpack_config.js', 
                                  'babel_config.js', 'jest_config.js', 'tsconfig.json']
            },
            'docs': {
    ...
    def check_file_content_type(self, file_path: str) -> bool:
        ...
        if file_path.startswith('project/src/') and file_path.endswith('.py'):
            try:
                ...
                js_patterns = ['import React', 'export default', 'const ', 'let ', 
    ...
    def check_file_encoding(self, file_path: str) -> bool:
        ...
        if not full_path.exists() or not file_path.endswith(('.py', '.md', '.json', '.yaml', '.yml')):
            return True
