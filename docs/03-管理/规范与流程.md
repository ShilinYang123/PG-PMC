...
本文旨在明确项目开发过程中的各项标准与流程，确保开发工作有序、高效进行，提升项目的可维护性与协作性。
...
*   仔细研读 "S:\3AI\docs\01-设计\开发任务书.md"。
*   结合当前任务看板，精准把握本次工作核心目标和交付成果。

### 2. 规划技术路径

*   参考 "S:\3AI\docs\01-设计\技术路线.md"。
...
*   数据存储路径禁止通过编码设置，而是通过配置文件及变量文件设置为：
    *   TaskManager 存储路径："项目根目录 /docs/02 - 开发 /tasks.json"
...
从代码仓库`S:\3AI\bak\github_repo`拉取最新代码，避免基于过旧版本开发，确保开发内容时效性。
...
将当日完成或有进展的代码全部提交到 Git 仓库`S:\3AI\bak`，遵循提交规范，编写清晰、准确提交说明，方便后续追溯与协作。
...
1.  调用"S:\3AI\tools\[check\_structure.py](http://check_structure.py/)"进行目录结构合规性检查；
    *   **检查方式**：对照目录结构标准清单对项目结构进行检查，扫描范围为项目根目录`S:\3AI`下除`\bak`和`\logs`目录及其子内容之外的所有内容。

2.  自动生成含违规项的自检报告。
    *   报告生成位置：`S:\3AI\logs\自检报告\`
    *   报告命名格式：`每日自检报告_YYYYMMDD - HHMMSS.md`
...
*   存放位置：`S:\3AI\docs\01 - 设计\目录结构标准清单.md`
*   清单涵盖内容：
...
*   **\bak 目录**：用于本地备份，其下子文件夹（如 "专项备份"、"迁移备份"）仅保留在本地，不推送到 Git 仓库。但目录结构本身需要在代码提交和系统备份时保持完整。

#### bak/ 和 logs/ 目录特殊处理规范

**目录结构保持规范：**
- `bak/` 和 `logs/` 目录的子目录结构必须按照 `docs/01-设计/目录结构标准清单.md` 中的规定保持
- 在 Git 提交、GitHub 代码提交、系统全量备份时，这两个目录的**完整目录结构**必须提交和备份
- 目录结构中的**具体内容文件**（如 `.zip` 备份文件、`.log` 日志文件、`.md` 报告文件等）**不提交、不备份**

**实施要求：**
1. **代码提交时**：提交空的目录结构，保持目录层级完整
2. **系统备份时**：备份目录结构，但排除具体内容文件
3. **脚本处理**：相关备份和提交脚本需要支持"仅目录结构"模式
4. **版本控制**：使用 `.gitkeep` 文件保持空目录在 Git 中的存在
...
│   └── finish_py_report_*.md      # finish脚本生成的工作报告
├── 检查报告/          # 各类检查和分析报告
│   ├── 自检报告/      # 自动生成的检查报告
│   │   ├── 每日自检报告_*.md
│   │   └── 目录结构检查报告_*.md
...
finish_py_report_20250621-165009.md

# 检查报告类
每日自检报告_20250621-165009.md
...
#### 5.2 配置文件更新
- `project_config.yaml`：`report_dir: logs/检查报告/自检报告`
- `structure_check_config.yaml`：`report_dir: logs/检查报告/自检报告`
